
function show(){
  return(
    <div className=" p-0 w-screen left-0 right-0 bg-slate-950 min-h-screen">
        {/* Your card content */}
        
      SDFSDFWEFW
      </div>
    
  )

}
export  const Content_card =() => {
  return (
    //container
    <div className="flex flex-col items-center bg-amber-700 w-screen h-screen p-0 relative text-center  mt-0 ">
      {/* First card */}
      <div className="  text-slate-50 flex flex-col  items-center justify-center p-0 w-screen left-0 right-0 bg-slate-950 min-h-screen ">
        {/*  card content */}
        
      <p>SDFSDFWEFW</p>
          {/*button */}
        <div>
          <button>
            ordernow

          </button>
          <button>
            demo drive
            
          </button>
        </div>
      </div>

      {/* Second card */}
      <div className=" flex flex-col p-0 w-screen left-0 right-0 bg-slate-300 min-h-screen ">
        {/* card content */}
        <p>sdfsdfsdfqwqeqweqweqwe</p>
      {/*button */}
        <div>
          <button>
            ordernow

          </button>
          <button>
            demo drive
            
          </button>
        </div>
        
      </div>
    </div>
  );
};

